...
export function StockRow({ stock, quote }: StockRowProps) {
  ...
  return (
    <tr className="border-b border-border/50 hover:bg-muted/50 transition-colors group" data-testid={`stock-row-${stock.ticker}`}>
      ...
      <td className="py-2.5 px-4 text-right text-muted-foreground text-xs align-top hidden lg:table-cell">
        <div className="truncate max-w-[120px]" title={stock.focus}>{stock.focus}</div>
      </td>
    </tr>
  );
}
